Feature: Create Spartan
  ...
  @TokenRequired
  @Happy
  Scenario Outline: Successfully create a new Spartan
    Given spartan endpoint is up and user is authenticated
    And spartan payload with  first name "<firstName>" last name "<lastName>" course stream name "<streamName>" and rest valid fields
    When user calls "spartan" endpoint with "POST" HTTP request
    Then the API responds with status code 204 and verify with getAllSpartans that the last spartan has first name "<firstName>" last name "<lastName>" course stream name "<streamName>"
    Examples:
      | firstName | lastName  | streamName |
      | firstName | lastName  | C# Test    |
      | Jonathan  | Williams  | Java Dev   |
